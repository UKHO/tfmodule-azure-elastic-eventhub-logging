resource "null_resource" "run_api" {
  provisioner "local-exec" {
    command = "curl -X POST -H 'Content-Type: application/json' -d '{\"name\": \"Agent policy 6\", \"description\": \"\", \"namespace\": \"default\", \"monitoring_enabled\": [\"logs\", \"metrics\"], \"inactivity_timeout\": 1209600, \"is_protected\": false}' http://${var.ELK_URL}/api/fleet/agent_policies?sys_monitoring=true"
    environment = {
